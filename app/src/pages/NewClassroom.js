...
function NewClassroom() {
  ...
  const handleAddNewClassroom = () => {
    if (Name.current.value && Type.current.value) {
      axios.post(apiurl + "addnewclassroom", {
    ...
    <div className="home_content">
      ...
      <div className="addroomwrap">
        <div className="wrap">
          <input
            type="text"
            className="input_room_id"
            placeholder="Terem azonosítója"
            ref={Name}
          />
          <select ref={Type} defaultValue={"Válassza ki a termet"} name="room" className="room_add_picker">
            ...
            {/* <option value="room0" selected disabled hidden>Válassza ki a termet</option> */}
          </select>
          <textarea
            name="Text1"
            cols="40"
            className="input_extra"
            rows="4"
            placeholder="Egyéb leírás"
            ref={Description}
          ></textarea>
          <div onClick={() => handleAddNewClassroom()} className="btn_save">Mentés</div>
          <br /><br /><br />
        </div>
      </div>
    </div>
